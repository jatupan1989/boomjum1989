...
Only filters which purpose is to block net requests are supported. ~~Element hiding filters and filters with the `$popup` option are ignored, since HTTPSB's primary purpose is to control net requests.~~ [Edit: element hiding filters are supported since [v0.9.4.0](/gorhill/httpswitchboard/wiki/Change-log#0940). The `$popup` option is ignored though -- Chromium has a built-in popup blocker.]
...
To give a glimpse of the better performance of HTTPSB over ABP in handling ABP-compatible filters, I measured that on average, **ABP evaluates 121 filters/URL**, while for the same benchmark **HTTPSB evaluates 17 filters/URL**.<sup>[1]</sup>
