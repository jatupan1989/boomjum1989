...
The major difference with ABP implementation is that only the CSS rules which are meaningful for the page are injected, which means a handful of them at most for a typical web page, and often none at all -- as opposed to ABP which injects 20,000 CSS rules ([as per ABP developer](https://blog.mozilla.org/nnethercote/2014/05/14/adblock-pluss-effect-on-firefoxs-memory-usage/comment-page-1/#comment-11173)).
