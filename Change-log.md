...
- Fixed <https://github.com/gorhill/httpswitchboard/issues/225>: "`slashdot.org` site is blocked when using `fanboy-annoyance.txt`".
    ...
    * This fix causes a whole set of ABP complex filters to move from HTTPSB's firewall-like engine to HTTPSB's custom ABP-filter engine, which results in an increase in the memory footprint of the extension when enabling ABP filtering.
    * No worry though, there are optimizations planned since the beginning for HTTPSB's custom ABP-filter engine which I expect will reduce memory footprint.
