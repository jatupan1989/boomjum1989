...
- New feature: per-page permissions. For example, this allows:
    ...
    * Net result: this disables `facebook.com` from tracking (like when you visit a web site in which `facebook.com` has assets embedded), except when visiting `facebook.com` web site using a secure connection (the `https` part).
