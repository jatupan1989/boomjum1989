...
- Fixed <https://github.com/gorhill/httpswitchboard/issues/225>: "`slashdot.org` site is blocked when using `fanboy-annoyance.txt`".
    ...
    * This fix causes a whole set of ABP complex filters to move from HTTPSB's firewall-like engine to HTTPSB's custom ABP-filter engine, which bumps up the memory footprint when enabling ABP filtering. No worry though, there are optimisations planned since the beginning for HTTPSB's custom ABP-filter engine which will reduce memory footprint.
