...
- Changed feature: You can now choose to auto-create domain- or site-level scopes (before only site-level scope was available).
    ...
    * For instance, it is quite common for a web site which requires logging in to use HTTP redirect directives from one subdomain to another.
        ...
        - With a site-level scope set to `login.example.com`, the login operation would fail because whatever whitelist rules created in the `login.example.com` scope do not apply to the `members.example.com` scope.
        ...
        - A real life example of issues raised by site-level scopes: <https://github.com/gorhill/httpswitchboard/issues/249>.
