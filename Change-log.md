...
- **New feature**: Auto-create temporary site-level scope (enable from *Settings* page)
    ...
    * Example of how this feature works (assuming out of the box settings):
        ...
        - Just for demonstration purpose: Whitelist "all" (top-left cell):
            * This very permissive rule would be unthinkable for security conscious users, however, in this case it applies **only** to web pages which URL address starts with `http://techcrunch.com/`.
    ...
    * Using the auto-creation of site-level scope feature is not very useful if you use HTTPSB in a whitelist-all/block-exceptionally mode.
